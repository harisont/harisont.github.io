...
Chronological log (if you prefer to browse by project, go to [the recap at the end of this page](#day-29-wrapping-up-for-real)):
...
- [Days 24-25: hack-a-ton](#days-24-25-hack-a-ton)
- [Day 26: a day to myself](#day-26-a-day-to-myself)
- [Day 27: July Adventure](#day-27-july-adventure)
- [Day 28: wrapping up... oh wait, not yet](#day-28-wrapping-up-oh-wait-not-yet)
- [Day 29: wrapping up... for real](#day-29-wrapping-up-for-real)
...
![A dependency tree for the sentence "It was not possible to add or remove nodes." being transformed into "Now it is possible to add or remove nodes!"](../assets/img/adventyr/wytiwys-add-rm.gif)
...
{% include image.html file="../assets/img/adventyr/book_stacks.png" description="Several attempts at photographing a stack of books in candlelight." %}
...
![A dependency tree for the sentence "you can't always search what you tree". On top of each node is the emoji of an open lock; the beginning and end of a sentence are marked by an anchor emoji.](../assets/img/adventyr/locks_anchors.png)
...
{% include image.html file="../assets/img/adventyr/rasperry_field_out.JPG" description="The outside of the Field Rasperry." %}

{% include image.html file="../assets/img/adventyr/rasperry_field_in.JPG" description="The inside of the Field Rasperry. In the jungle, the mighty jungle, the hard drive (as well as the Pi itself) sleeps tonight." %}
...
![Live treview in VSCodium](../assets/img/adventyr/treview.gif)
...
![A dependency tree with a bunch of decorations that allow the user to express order and adjacency constraints.](../assets/img/adventyr/xmas-deptree.png)
...
{% include image.html file="../assets/img/adventyr/capas.jpg" description="My little photo book standing on the shoulders of giants in my cousin's shelf." %}
...
```shell
alias yogurt="yay" 
# used to be an alias for yaourt, which is impossible to type. Stays for 
# historical reasons

alias grab="history | grep" 
# most used alias ever (I tend to forget it's an alias), even though the name 
# should have been something more archaeological
...
# recommended use: COMMAND_THAT_TAKES_FOREVER && pling. The specific sound file
# is from the lovely videogame Hiddenfolks, where all sound effects are 
# recordings of someone saying the sounds
...
alias skit="fuck"
alias nope="fuck"
# various ways to fix less predictable typos, regardless of the language my 
# brain is thinking in (I don't tend to swear in Italian, so ops will have 
# to do)
...
## Days 24-25: hack-a-ton
As I mentioned earlier, my friend C. and I had plans for a two-day xmas hackathon.
Literally, though, a _hack-athon_ is a hacking marathon, and marathons are often competitive. 
Often, the goal is typically to build and showcase a working prototype of an application.
This is not quite what we were aiming for: we had no competitors and no clear goal, just a vague project idea. 
So ours is more of a _hack-a-ton_, which will be followed by a series of _hack-a-bits_.

The basic idea is quite simple. 
C. (who's currently _actually_ learning web development) and I often cook together and I constantly complain about recipe websites:[^3] wordy to the point of involuntary comedy, filled with annoying popups and images that I never look at anyway and, most importantly, utterly non-algorithmic!
It's funny how recipes are often used as metaphors for imperative programs and yet they suddenly instruct you to "now add chopped garlic" when the only previous mention of garlic was kilometers away, far up in the ingredient list. 
Do they think the garlic chops itself? 
Is this some kind of lazy evaluation?
This lack of structure is especially frustrating when cooking together, because it hides any opportunities for parallelization.

That's why we decided to put together a recipe website by programmers, for programmers.
Recipes are stored in structured files written in a subset of Markdown, possible multithreading ~~is~~ will be clearly visualized based on the number of available CPUs (aka people in the kitchen) and the UI is inspired by command-line applications. 
No useless text or images are ever shown unless the `--verbose` flag is enabled.

{% include image.html file="../assets/img/adventyr/cc-home.png" description="WIP Concurrent Cooking homepage; styling inspired by htop." %}

{% include image.html file="../assets/img/adventyr/cc-recipe.png" description="Very WIP Concurrent Cooking recipe page; styling inspired by command-line Markdown previewers such as glow." %}

Of course, nothing is ready. 
But we did hack a ton, learned new things and had fun.

## Day 26: a day to myself
The hack-a-ton was fun, but (by definition) a lot, so no code today.
Spent the day by myself sleeping, roller blading (sunny afternoon, yay!), cleaning the apartment and going to the cinema to see the latest Jafar Panahi movie.
Only after I came back home I found some energy to sit at my desk and transcribe some more of the bikepacking logbook.
The next couple (pre-travel) days will be hectic, but publishing it as a blogpost as part of my December Adventure doesn't strike me as unthinkable.
We'll see how things go.

## Day 27: July Adventure
Published the full [logbook transcript](https://harisont.github.io/2025/12/27/ctc.html) of the Denmark bikepacking trip I went on last July!

I wanted to horizontally align some photos and figured I could use Markdown tables to do that.
Then I realized that I could use (one-item) tables for _every_ picture with a caption.
Mind blown!
Guess I can retire my complicated way of adding captions to pictures (which I also used earlier in this adventure log).
From now on, my captioned photos will look like this:

| My favorite photo from my July adventure                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------- |
| ![Silhouette of a little bird standing on the top of our tent seen from the inside of a tent](../assets/img/ctc/6-bird.JPG) |

Right now, I'm actually really happy with having the "caption" on _top_ of the image, but if I change my mind I suppose I can do

| ![Silhouette of a little bird standing on the top of our tent seen from the inside of a tent](../assets/img/ctc/6-bird.JPG) |
| --------------------------------------------------------------------------------------------------------------------------- |
| My favorite photo from my July adventure, but this time the caption is underneath                                           |

as well... I feel like [a genius](https://youtu.be/5-l51Lmk1H4?si=n8vzcK6YH9LIBXC4&t=40).

Finalizing the blog post, though, was 100% procrastinating on packing and doing the work I should complete before leaving for two weeks.
I suppose tomorrow's adventure will have to be a work-related one, but it's actually good that my brain is in holiday mode.
Sometimes it takes longer to decompress.

## Day 28: wrapping up... oh wait, not yet
Thought I'd wrap up the adventure today as I'm leaving for a couple weeks tomorrow and was considering leaving my laptop behind.
Next term, I'll be writing my PhD thesis + a long paper crucial to the thesis itself (not in this order) and, at the same time, supervising a Master's thesis and co-teaching a course in Computational Syntax.
In other words, the first half of the new year will be intense and I know that, even though I feel unusually energized from working on all of the projects I've tackled in this adventure, I need to pace myself and take a real break. 
Not only from _actual_ work, however fun (cf. wytiwys), but also from liminal academic work (such as treview) and maybe even from other programming and writing projects (such as Concurrent Cooking and "proper" blog posts) that are definitely in the hobby territory but keep my brain in a similar mode of operation... and ideally from screens altogether, although that's easier in the summer.
On the other hand, I feel the urge to close all side quests so that I can focus on my four main tasks next semester and I am supervising some students _right now_ as well, meaning I cannot disappear completely until mid January.
After much debating, I decided that the laptop is coming with me and that an `x`-day break where `10 <= x < 15` will have to do.
On the bright side, some of the remaining work is fun, which provides me with an excuse for one or two more days of adventure.

As for today, I did a little bit of everything:

- I implemented the UI elements (but not the underlying functionality) for hiding/displaying (un)wanted linguistic features in wytiwys:
  ![A dependency tree for the sentence "What You See Is Just Part Of What You Get". Right above it there is a row of checkboxes to select which linguistic features to display](../assets/img/adventyr/wysijpowyg.png)
  Once this is complete, I will be done with 3 out of the 3 main features I set myself to add and will put the project on hiatus until I'm back from my holidays
- I packed for my upcoming trip, part of which is digital. I assembled issue #11 of _hmm_ (see [day 13](#day-13-hmm) for more context) and I went through my digital music library to select some albums to bring along on my phone. I chose:

  - a new album: _Real life_ (Kinga Głyk)
  - two almost forgotten albums:  _Alice_ (Tom Waits) and _Pulchra es_ (Il Ricercar Continuo)
  - a comfort album: _R.E.M. Unplugged 1991-2001_
  - a background album: _Ambient 1 - music for airports_ (Brian Eno)
 
  Doing this brings me back to the portable CD player times, which is both good and bad. What is definitely 100% bad are the [absurd limitations of Nextcloud for Android](https://help.nextcloud.com/t/how-is-internal-two-way-sync-supposed-to-work/212610). I naively thought that uploading these albums to Nextcloud would automatically imply that (modulo some "yes I do really want to have these large files on this device"s) they would end up in a normal folder of my Android smartphone where I could do whatever I wanted with them, but no. Apparently I can only access them and download them one by one through the stupid app. What a disappointment, this could have been a simple medium-term solution for listening to my "current rotation" here (on my computer), there (on my laptop) and everywhere
- I finalized (but not yet published) my "2025 stdin" blog post, which is basically just a list of books I read, movies I watched and podcast listened to this year. It will become public on December 31.

## Day 29: wrapping up... for real
It is very, very early in the morning, so early that it'd be physically hard for me to wake up now... had I gone to sleep in the first place.
It suddenly got too late to go to bed without risking missing my flight and I had another change of heart: to keep myself awake, I did the only work that absolutely a. requires sitting in front of a computer and b. needs to be done within the next few days. 
I plan to crash as soon as I board the plane and have two as restful and computer-free[^4] weeks as possible.

So, that's the end of my adventure.
I worked on a lot of different things this month:

- __this website__, including three different blog posts (days [1](#day-1-plans), [6](#day-6-chores-and-literature), [7](#day-7-more-literature), [11](#day-11-small-website-updates), [12](#day-12-hobby-as-work-work-as-hobby), [20](#day-20-bikepacking-logbook-transcript--tripod-thing), [21](#day-21-more-treviewing-and-transcribing), [22](#day-22-decorating-the-xmas-deptree), [26](#day-26-a-day-to-myself), [27](#day-27-july-adventure), [28](#day-28-wrapping-up-oh-wait-not-yet))!
- __wytiwys__, a dependency tree editor for What You Tree Is What You Search corpus queries (days [2](#day-2-wytiwys-wth), [4](#day-2-wytiwys-wth), [5](#day-5-addremove-node-complete-and-new-website-section), [8](#day-8-what-you-see-is-what-you-wish-you-could-get), [17](#day-17-slow-almost-still), [22](#day-22-decorating-the-xmas-deptree), [28](#day-28-wrapping-up-oh-wait-not-yet))
- __hmm__, my personal digital magazine (days [13](#day-13-hmm), [28](#day-28-wrapping-up-oh-wait-not-yet))
- the __Field Raspberry__ (Pi) (days [13](#day-13-hmm), [14](#day-14-raspberry-fields-forever), [15](#day-15-tired-enough-to-look-up-games))
- DIY gifts: a __photo book__ and a __board game__ (days [16](#day-16-round-robining), [23](#day-23-diy-gifts-and-zshrc))
- __treview__, a CoNLL-U to HTML converter for live previewing UD trees in VSCod* (days [18](#day-18-we-dont-need-no-code-extension), [19](#day-19-better-treview), [21](#day-21-more-treviewing-and-transcribing))
- __Concurrent Cooking__, a recipe website by and for (parallel) programmers (days [24-25](#days-24-25-hack-a-ton))
- other random stuff (days [3](#day-3-latex-macros), [9](#day-9-music), [10](#day-10-work-after-work-and-after-after-work-work), [23](#day-23-diy-gifts-and-zshrc), [28](#day-28-wrapping-up-oh-wait-not-yet))

A few of these projects are now complete, some I made good progress on, some I've just gotten started with and some are an eternal work in progress.
All of them brightened this dark December.
...
[^3]: Excepts [grimgrains](https://grimgrains.com/site/home), I like grimgrains.
[^4]: Actually, doing computer work while visiting my family is not really a problem - my dad uses the same Linux distro as I do (because I set it up strategically)! Not having my window manager and development environments ready, though, will hopefully provide a useful disincentive from working on anything major that I'm neither obliged nor really longing to do.